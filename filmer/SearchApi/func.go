...
func cutWords(sentence []byte) []string {
    ...
    segementer.LoadDictionary("/home/jacksie/go/src/github.com/huichen/sego/data/dictionary.txt")
...
//先预处理查询没有重复的id，再返回
func doSearch(words []string,resultP *model.GroundInfos) error {
    var sum int
...
func doSearch(words []string,resultTagp *tag) error {
    var sum int
    var m1 map[string]bool
    var lock sync.Mutex
    l := len(words)
