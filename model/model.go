...
func GetReview(user_id string) ([]Review, error) {
	var reviews []Review
	if err := Db.Self.Table("user_review").Where("user_id=?", user_id).Find(&reviews).Error; err != nil {
...
func NewAlbum(album Album, user_id string) int {
	var lastalbum Album
	if err := Db.Self.Table("album").Where("user_id=?", user_id).Order("album_id desc").First(&lastalbum).Error; err != nil {
		//没找到记录也会报错
		lastalbum.AlbumID = 0
	}
...
func GetAlbums(user_id string) ([]Album, error) {
	var albums []Album
	if err := Db.Self.Table("album").Where("user_id=?", user_id).Find(&albums).Error; err != nil {
